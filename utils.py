...
def accuracy(output, target, topk=(1,)):
	...
	res = []
	for k in topk:
    	# correct_k = correct[:k].view(-1).float().sum(0) # RuntimeError: view size is not compatible with input tensor's size and stride (at least one dimension spans across two contiguous subspaces). Use .reshape(...) instead.
